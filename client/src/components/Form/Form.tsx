...
const Form = ({currentId, setCurrentId}:any) => {
    ...
    const  handleSubmit=(e:any)=>{
        ...
        if(currentId){
            ...
            dispatch(Services.createPosts({...postData, name:userData?.result?.name}, history));
            clear();
        }
    };
    ...
    return (
        <Paper elevation={6} className={classes.paper}>
            <form autoComplete="off" noValidate className={`${classes.root} ${classes.form}`} onSubmit={handleSubmit}>
                ...
                <TextField name="message" variant="outlined" size="small" label="Message" fullWidth multiline rows={6} value={postData.message} onChange={(event)=>setPostData({ ...postData, message: event.target.value})}/>
